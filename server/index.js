...
const ALLOWED_KEY_PREFIXES = ['fleet-invoice-', 'paperwork-job-link-', 'paperwork-upload-', 'compliance-doc-'];
...
const SEED_USERS = [
  { email: 'aaron.henson@chhj.com' },
  { email: 'administrative.assistantaug@chhj.com' }
];
  ...
  try {
    ...
    for (const seed of SEED_USERS) {
      if (!users[seed.email]) {
        ...
        users[seed.email] = { passwordHash, mustReset: true };
        changed = true;
      }
    }
    if (changed) {
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ token, mustReset: !!user.mustReset });
  } catch (err) {
